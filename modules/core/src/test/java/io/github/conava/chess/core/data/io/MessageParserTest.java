...
class MessageParserTest {
    ...
    void parse_extractsCorrectMessageType() {
        ...
        assertEquals(MessageType.MOVE, msg.type(), "parse() must map the prefix to the correct MessageType");
    }

    @Test
    void parse_extractsCorrectContent() {
        Message msg = MessageParser.parse("MOVE:from=e2 to=e4");
        assertEquals("from=e2 to=e4", msg.content(), "parse() must preserve the content portion unchanged");
    }
    ...
    void parse_handlesEmptyContent() {
        ...
        assertEquals("", msg.content(), "parse() must accept an empty content string after the colon");
    }
    ...
    void parse_contentWithColonIsNotFurtherSplit() {
        ...
        assertEquals("key=val:extra", msg.content(), "parse() must not split on colons inside the content portion");
    }

    @Test
    void parse_unknownMessageTypeThrows() {
        assertThrows(IllegalArgumentException.class, () -> MessageParser.parse("UNKNOWN:something"), "parse() must throw when the type prefix is not a valid MessageType name");
    }
    ...
    void serialize_producesCorrectWireString() {
        ...
        assertEquals("MOVE:from=e2 to=e4", MessageParser.serialize(msg), "serialize() must produce '<TYPE>:<content>'");
    }

    @Test
    void serialize_emptyContentProducesTrailingColon() {
        Message msg = new Message(MessageType.SUCCESS, "");
        assertEquals("SUCCESS:", MessageParser.serialize(msg), "serialize() of empty-content message must end with a colon");
    }
    ...
    void parseAndSerialize_roundTrip() {
        ...
        assertEquals(wire, serialized, "serialize(parse(wire)) must reproduce the original wire string");
    }
    ...
    void serializeAndParse_roundTrip() {
        ...
        assertEquals(original.type(), reparsed.type(), "parse(serialize(msg)) must reproduce the original MessageType");
        assertEquals(original.content(), reparsed.content(), "parse(serialize(msg)) must reproduce the original content");
    }
    ...
    void getParameterValue_returnsCorrectValueForExistingKey() {
        Message msg = new Message(MessageType.MOVE, "from=e2 to=e4");
        assertEquals("e2", msg.getParameterValue("from"), "getParameterValue('from') must return 'e2'");
    }

    @Test
    void getParameterValue_returnsCorrectValueForSecondKey() {
        Message msg = new Message(MessageType.MOVE, "from=e2 to=e4");
        assertEquals("e4", msg.getParameterValue("to"), "getParameterValue('to') must return 'e4'");
    }

    @Test
    void getParameterValue_returnsNullForMissingKey() {
        Message msg = new Message(MessageType.MOVE, "from=e2 to=e4");
        assertNull(msg.getParameterValue("piece"), "getParameterValue() must return null when the key is not present");
    }

    @Test
    void getParameterValue_emptyContentReturnsNull() {
        Message msg = new Message(MessageType.SUCCESS, "");
        assertNull(msg.getParameterValue("anything"), "getParameterValue() on empty content must return null");
    }
    ...
    void message_nullTypeThrowsIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> new Message(null, "content"), "Message constructor must reject a null type");
    }

    @Test
    void message_nullContentThrowsIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> new Message(MessageType.SUCCESS, null), "Message constructor must reject null content");
    }
}
